#!/usr/bin/env python3
"""
TASK 1a (QUALITY ASSURANCE): Model Submission Ranking and Statistical Testing Pipeline

Input: single cleaned long-form CSV of raw performance metric values for a task 1a, 
...
Output: Ranked models for task 1a in CSV
"""
...
JSON_PATH = Path("../config/task1a_metric_rank_directions.json")
OUTPUT_SCOREBOARD_DIR = Path("../final_scoreboards")
RAW_METRICS_FOLDER_PATH = Path("../task_raw_metrics")
task1a_csv_path = RAW_METRICS_FOLDER_PATH/"task1a_raw_metrics.csv"
ALPHA = 0.05
...
def main():
    """
    use raw performance metric values in CSV for task 1a to calculate team model rankings & run Wilcoxon signed rank statistical testing to check statisstical significance of model rankings
    """
    ...
    # load csv of raw metric values for task 1a into pd df
    if not os.path.exists(task1a_csv_path):
        print(f"Critical Error: Task 1a Raw metrics CSV missing at: {task1a_csv_path}")
        sys.exit(1)
    task1a_raw_metric_df = pd.read_csv(task1a_csv_path)
    
    # sanity check CSV cols & names & unique task ID for CSV
    required_cols = {'Subject_ID', 'Model_ID', 'Task_ID', 'Performance_Metric', 'Value'}
    if not required_cols.issubset(task1a_raw_metric_df.columns):
        print(f"Critical Error: Col names in Task 1a raw metrics CSV at {task1a_csv_path} don't match required names: {required_cols}")
        sys.exit(1)
    elif (len(task1a_raw_metric_df['Task_ID'].unique()) > 1) or (task1a_raw_metric_df['Task_ID'].unique()[0] != "Task1a"):
        print(f"Critical Error: Task 1a raw metrics CSV at {task1a_csv_path} contains more than one task ID: {task1a_raw_metric_df['Task_ID'].unique()}")
        sys.exit(1)

    # extract task ID from column in file
    task_id = task1a_raw_metric_df['Task_ID'].unique()[0]

    # run ranking & statistical testing pipeline on task df
    task_model_rankings_testing(task1a_raw_metric_df, task_id, metric_directions)
